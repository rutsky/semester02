...
static long _getLevelOffset( int base, int level )
{
  if (level == 0)
    ...
    return base * (Power(base, level) - 1.0) / (base - 1.0);
...
static void _getPos( int base, long offset, int *level, long *n )
{
  ...
  lev = log(1 + offset * (base - 1.0) / base) / log(base);
  if (level != NULL)
   *level = lev;
  if (n != NULL)
...
{
  return heap->data + heap->size * offset;
...
void *GetElem( Heap *heap, int level, int n )
{
  ...
  return heap->data + heap->size * (_getLevelOffset(heap->base, level) + n);

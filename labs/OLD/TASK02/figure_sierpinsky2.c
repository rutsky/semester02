...
static int SierpinskyRenderLines( Figure *fig, Line *buffer, int nlines, int level )
{
  ...
  buffer[lcounter].x0 = ((SierpinskyData *)fig->data)->start.x;
  buffer[lcounter].y0 = ((SierpinskyData *)fig->data)->start.y;
  buffer[lcounter].x1 = t1.x;
  buffer[lcounter++].y1 = t1.y;
  if (lcounter >= nlines)
    return lcounter;
    
  buffer[lcounter].x0 = ((SierpinskyData *)fig->data)->start.x;
  buffer[lcounter].y0 = ((SierpinskyData *)fig->data)->start.y;
  buffer[lcounter].x1 = t2.x;
  buffer[lcounter++].y1 = t2.y;
  if (lcounter >= nlines)
    return lcounter;
  
  buffer[lcounter].x0 = t1.x;
  buffer[lcounter].y0 = t1.y;
  buffer[lcounter].x1 = t2.x;
  buffer[lcounter++].y1 = t2.y;
  if (lcounter >= nlines)
    return lcounter;
  ...
  for (l = 0; l < level; l++)
  {
    ...
    for (i = 0; i < nelems; i++)
    {
      ...
      buffer[lcounter].x0 = t2.x;
      buffer[lcounter].y0 = t2.y;
      buffer[lcounter].x1 = t3.x;
      buffer[lcounter++].y1 = t3.y;
      if (lcounter >= nlines)
        return lcounter;

      buffer[lcounter].x0 = t3.x;
      buffer[lcounter].y0 = t3.y;
      buffer[lcounter].x1 = t1.x;
      buffer[lcounter++].y1 = t1.y;
      if (lcounter >= nlines)
        return lcounter;
...
Figure *SierpinskyFigInit( int maxLoD, Triangle *tr )
{
  Figure *fig;
  SierpinskyHeapData *p;
  ...
  VecSet(&((SierpinskyData *)fig->data)->start, tr->x0, tr->y0);
  VecSet(&((SierpinskyData *)fig->data)->v1, tr->x1, tr->y1);
  VecSub(&((SierpinskyData *)fig->data)->v1, &((SierpinskyData *)fig->data)->start);
  VecSet(&((SierpinskyData *)fig->data)->v2, tr->x2, tr->y2);
...
/* END OF 'figure_sierpinsky.c' FILE */
